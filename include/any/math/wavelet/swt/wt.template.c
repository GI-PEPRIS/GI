...
int CAT(TYPE, _downcoef_axis)(const TYPE * const restrict input, const ArrayInfo input_info,
                              ...
                              const SWT_Wavelet * const restrict wavelet, const size_t axis,
    ...
    if (make_temp_input)
        if ((temp_input = (float*)malloc(input_info.shape[axis] * sizeof(TYPE))) == NULL)
            goto cleanup;
    if (make_temp_output)
        if ((temp_output = (float*)malloc(output_info.shape[axis] * sizeof(TYPE))) == NULL)
            goto cleanup;
...
int CAT(TYPE, _idwt_axis)(const TYPE * const restrict coefs_a, const ArrayInfo * const a_info,
                          ...
                          TYPE * const restrict output, const ArrayInfo output_info,
                          const SWT_Wavelet * const restrict wavelet,
    ...
    if (make_temp_coefs_a)
        if ((temp_coefs_a = (float*)malloc(a_info->shape[axis] * sizeof(TYPE))) == NULL)
            goto cleanup;
    if (make_temp_coefs_d)
        if ((temp_coefs_d = (float*)malloc(d_info->shape[axis] * sizeof(TYPE))) == NULL)
            goto cleanup;
    if (make_temp_output)
        if ((temp_output = (float*)malloc(output_info.shape[axis] * sizeof(TYPE))) == NULL)
            goto cleanup;
...
int CAT(TYPE, _idwt)(const TYPE * const restrict coeffs_a, const size_t coeffs_a_len,
                     ...
                     TYPE * const restrict output, const size_t output_len,
                     const SWT_Wavelet * const restrict wavelet, const MODE mode){
    size_t input_len;
...
int CAT(TYPE, _swt_)(TYPE input[], index_t input_len,
    ...
    if(level > 1){
        ...
        e_filter = (float*)wtcalloc(e_filter_len, sizeof(TYPE));
        if(e_filter == NULL)
            return -1;
 ...
 */
int CAT(TYPE, _swt_a)(TYPE input[], index_t input_len, SWT_Wavelet* wavelet,
                      TYPE output[], index_t output_len, int level){
 ...
 * input - approximation coeffs from upper level or signal if level == 1
 */
int CAT(TYPE, _swt_d)(TYPE input[], index_t input_len, SWT_Wavelet* wavelet,
                      TYPE output[], index_t output_len, int level){
