...
struct TF_CLASS EqualGraphDefOptions {
...
TF_API bool EqualGraphDef(const GraphDef& actual, const GraphDef& expected,
...
TF_API uint64 GraphDefHash(const GraphDef& gdef,
...
TF_API bool EqualNodeDef(const NodeDef& actual, const NodeDef& expected, string* diff,
                  const EqualGraphDefOptions& options = {});
...
// guaranteed to be stable only for a given binary. In particular, one should
// probably not persist the returned value.
TF_API uint64 NodeDefHash(const NodeDef& ndef,
                   const EqualGraphDefOptions& options = {});
...
// difference.
TF_API bool EqualRepeatedNodeDef(const protobuf::RepeatedPtrField<NodeDef>& actual,
...
// guaranteed to be stable only for a given binary. In particular, one should
// probably not persist the returned value.
TF_API uint64 RepeatedNodeDefHash(const protobuf::RepeatedPtrField<NodeDef>& ndefs,
                           const EqualGraphDefOptions& options = {});
