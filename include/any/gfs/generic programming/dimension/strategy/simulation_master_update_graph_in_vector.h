...
#ifndef ANYWAY_RCF_DIMENSION_SIMULATION_MASTER_UPDATE_GRAPH_IN_VECTOR_H
#define ANYWAY_RCF_DIMENSION_SIMULATION_MASTER_UPDATE_GRAPH_IN_VECTOR_H
	...
	namespace rcf
	{
		namespace detail
		{
			namespace strategy
			{
				...
				// services
				//
				struct simulation_master_update_graph_in_vector : public default_strategy
				{
					//
					template
						<
					...
					static bool apply(Master &master, Graph &graph)
					{
						...
						try
						{
							...
							masterimpl->UpdateTopoGraph(graph.m_job_graph_idx, graph.m_job_nodes_in_topo_order);
