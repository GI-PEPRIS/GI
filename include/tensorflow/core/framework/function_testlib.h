...
TF_API GraphDef GDef(gtl::ArraySlice<NodeDef> nodes,
...
// x:T -> x * 2.
TF_API FunctionDef XTimesTwo();

// x:T -> cpu(x * 2) + cpu(x * 3).
TF_API FunctionDef TwoDeviceTimesFive();

// x:T -> cpu(x * 2), gpu(x * 3).
TF_API FunctionDef TwoDeviceMult();

// cpu(x):T, gpu(y):T -> cpu(x * 2), gpu(y * 3).
TF_API FunctionDef TwoDeviceInputOutput();

// Function taking a list of Tensors as input.
TF_API FunctionDef FuncWithListInput();

// Function returning a list of Tensors as output.
TF_API FunctionDef FuncWithListOutput();

// x:T -> x + x.
TF_API FunctionDef XAddX();

// x: T, y:T -> x + y.
TF_API FunctionDef XAddY();

// x:T -> x * 2, where x is int32.
TF_API FunctionDef XTimesTwoInt32();

// x:T -> (x * 2) * 2.
TF_API FunctionDef XTimesFour();

// x:T -> ((x * 2) * 2) * 2.
TF_API FunctionDef XTimes16();

// w:T, x:T, b:T -> MatMul(w, x) + b
TF_API FunctionDef WXPlusB();

// x:T -> x:T, T is a type which we automatically converts to a bool.
TF_API FunctionDef NonZero();

// x: T -> bool.
TF_API FunctionDef IsZero();

// x: T -> int64
TF_API FunctionDef RandomUniform();

// x:T, y:T -> y:T, x:T
TF_API FunctionDef Swap();

// x:T, y:T -> y:T, x:T, the body has no nodes.
TF_API FunctionDef EmptyBodySwap();

// x:float, y:resource -> y:resource, 2*x:float.
TF_API FunctionDef ResourceOutput();

// x:resource -> x:resource
TF_API FunctionDef ResourceIdentity();

// x:resource -> y:float.
TF_API FunctionDef ReadResourceVariable();

// Contains malformed control flow which can't be run by the executor.
TF_API FunctionDef InvalidControlFlow();

// x:T -> x <= N.
TF_API FunctionDef LessThanOrEqualToN(int64 N);

// x:T, y:T -> x+1, x*y
TF_API FunctionDef XPlusOneXTimesY();

// x:T, y:T -> x <= N
TF_API FunctionDef XYXLessThanOrEqualToN(int64 N);

// x: T -> bool
TF_API FunctionDef RandomUniformLess();

// x:T -> y: TensorSliceDatasetOp::Dataset
TF_API FunctionDef MakeTensorSliceDataset();

// x:T -> y: T, idx: out_idx
TF_API FunctionDef Unique();

TF_API void FunctionTestSchedClosure(std::function<void()> fn);
