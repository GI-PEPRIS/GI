...
//
class ANYTHREAD_CLASS JTCThread : public virtual JTCRefCount
{
public:
	...
	void stopOtherThread(HANDLE handle);
...
public: // No friend decl for extern "C functions
	...
	//
	// Private constructor(Change to public because thread pool need to set the main thread) for booting JTC.
	//
...
//
class ANYTHREAD_CLASS JTCInitialize
{
    //
...
public:
	...
	//Return static jtcinitialze instance
	static JTCInitialize* GetInstance();

	// return main thread
	JTCThreadHandle GetMainThread();
