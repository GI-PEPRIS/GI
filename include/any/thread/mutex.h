...
//
class ANYTHREAD_CLASS JTCMutex
{
public:
...
public: //private: //herman

    //
    // Hide copy constructor and assignment operator.
    //
    JTCMutex(const JTCMutex&other){*this = other;}
    
    const JTCMutex&
    operator=(const JTCMutex&other)
	{
		if(this != &other)
		{
			m_lock = other.m_lock;
...
// locked.
//
class ANYTHREAD_CLASS JTCRecursiveMutex
{
public:
...
//
// This class use semaphore to lock and unlock 
class ANYTHREAD_CLASS JTCTimedMutex
{
public:

	JTCTimedMutex():m_lock(1)
	{
		//((JTCTimedMutex*)this) -> m_lock(1);
		//JTCSemaphore lock = ((JTCTimedMutex*)this) -> m_lock;
	}
	...
	{
		//JTCSemaphore lock = ((JTCTimedMutex*)this) -> m_lock;
		((JTCTimedMutex*)this) -> m_lock.wait();
		return true;
	}
	...
	// Lock the mutex.
	//
	bool lock(long timeout) const
	{
		//JTCSemaphore lock = ((JTCTimedMutex*)this) -> m_lock;
		((JTCTimedMutex*)this) -> m_lock.wait(timeout);
		return true;
	}
	...
	{
		//JTCSemaphore lock = ((JTCTimedMutex*)this) -> m_lock;
		((JTCTimedMutex*)this) -> m_lock.post();
		return true;
	}
...
//	}
	...
	JTCSemaphore m_lock; // semaphore
private:

	//
	// Hide copy constructor and assignment operator.
	//
	JTCTimedMutex(const JTCTimedMutex&);

	void operator=(const JTCTimedMutex&);
