 ...
 */
#ifndef __ANYDB_SQLITE_ACCESSORIMPL_H__
#define __ANYDB_SQLITE_ACCESSORIMPL_H__
#include "sqlite3.h"
...
namespace AnyDB
{
	...
	class CAnySqliteRecordset;
	
	/*!
	*
	**/
	class ANYDB_CLASS CAnySqliteAccessorImpl : public CAnyAccessorImpl, public JTCMonitor
	{
		// Relatives
		friend class CAnyAccessorImpl;

		// For those records in memory respectively for each table
		typedef std::map<const std::string, CAnySqliteRecordset*> RecordsetMap;
	...
	protected:
		...
		*
		**/
		CAnySqliteAccessorImpl(const DBSourceDesc &source);

		/*!
		*
		**/
		virtual ~CAnySqliteAccessorImpl();
	...
	private:
		...
		// Current record set
		CAnySqliteRecordset *m_curRecordset;
