...
TF_API size_t FastInt32ToBufferLeft(int32 i, char* buffer);    // at least 12 bytes
TF_API size_t FastUInt32ToBufferLeft(uint32 i, char* buffer);  // at least 12 bytes
TF_API size_t FastInt64ToBufferLeft(int64 i, char* buffer);    // at least 22 bytes
TF_API size_t FastUInt64ToBufferLeft(uint64 i, char* buffer);  // at least 22 bytes

// Required buffer size for DoubleToBuffer is kFastToBufferSize.
// Required buffer size for FloatToBuffer is kFastToBufferSize.
TF_API size_t DoubleToBuffer(double value, char* buffer);
TF_API size_t FloatToBuffer(float value, char* buffer);

// Convert a 64-bit fingerprint value to an ASCII representation.
TF_API string FpToString(Fprint fp);
...
// returns false.
TF_API bool StringToFp(const string& s, Fprint* fp);
...
TF_API StringPiece Uint64ToHexString(uint64 v, char* buf);
...
// returns false.
TF_API bool HexStringToUint64(const StringPiece& s, uint64* v);

// Convert strings to 32bit integer values.
// Leading and trailing spaces are allowed.
// Return false with overflow or invalid input.
TF_API bool safe_strto32(StringPiece str, int32* value);

// Convert strings to unsigned 32bit integer values.
// Leading and trailing spaces are allowed.
// Return false with overflow or invalid input.
TF_API bool safe_strtou32(StringPiece str, uint32* value);

// Convert strings to 64bit integer values.
// Leading and trailing spaces are allowed.
// Return false with overflow or invalid input.
TF_API bool safe_strto64(StringPiece str, int64* value);

// Convert strings to unsigned 64bit integer values.
// Leading and trailing spaces are allowed.
// Return false with overflow or invalid input.
TF_API bool safe_strtou64(StringPiece str, uint64* value);

// Convert strings to floating point values.
// Leading and trailing spaces are allowed.
// Values may be rounded on over- and underflow.
// Returns false on invalid input or if `strlen(value) >= kFastToBufferSize`.
TF_API bool safe_strtof(StringPiece str, float* value);

// Convert strings to double precision floating point values.
// Leading and trailing spaces are allowed.
// Values may be rounded on over- and underflow.
// Returns false on invalid input or if `strlen(value) >= kFastToBufferSize`.
TF_API bool safe_strtod(StringPiece str, double* value);
