 ...
 */
#ifndef __ANYBASIN_HYPER_DATA_THREADED_ITER_H__
#define __ANYBASIN_HYPER_DATA_THREADED_ITER_H__

//
#include "hyperdata.h"

//
#ifndef USE_CX11_FEATURE
#include "anythread\jtc.h"
#endif
...
namespace AnyBasin
{
	...
	class HyperThreadedIter : public HyperDataIter<DType> 
	{
	 public:
	  /*!
...
#ifndef USE_CX11_FEATURE
	  template<typename DType>
	  class JTCProducerThread : public JTCThread
	  {
	  public:
		  HyperThreadedIter<DType> *m_parent;
		  std::function<bool(DType **)> m_next;
		  std::function<void()> m_beforefirst;

		  bool queue_not_empty()
		  {
			  if (m_parent->producer_sig_ == kProduce) 
			  {
				  bool ret = !m_parent->produce_end_ &&
					  (m_parent->queue_.size() < m_parent->max_capacity_ || m_parent->free_cells_.size() != 0);
				  return ret;
			  } 
			  else 
			  {
		  ...
		  void run () 
		  {
			  while (true) 
			  {
				  ...
				  {
					  // lockscope
					  JTCSyncT<JTCMutex> lock(m_parent->mutex_);
					  ++m_parent->nwait_producer_;

					  while(!queue_not_empty())
					  {
						  m_parent->producer_cond_.wait(m_parent->mutex_);
					  }			

					  --m_parent->nwait_producer_;
					  if (m_parent->producer_sig_ == kProduce) 
					  {
						  if (m_parent->free_cells_.size() != 0) 
						  {
							  cell = m_parent->free_cells_.front();
							  m_parent->free_cells_.pop();
						  }
					  } else if (m_parent->producer_sig_ == kBeforeFirst) {
						  // reset the producer
						  m_beforefirst();
						  // cleanup the queue
						  while (m_parent->queue_.size() != 0) {
							  m_parent->free_cells_.push(m_parent->queue_.front());
							  m_parent->queue_.pop();
						  }
						  // reset the state
						  m_parent->produce_end_ = false;
						  m_parent->producer_sig_processed_ = true;
						  m_parent->producer_sig_ = kProduce;
						  // notify consumer that all the process as been done.
						  m_parent->mutex_.unlock();
						  m_parent->consumer_cond_.broadcast();
						  continue;
					  } else {
						  // destroy the thread
						  assert(m_parent->producer_sig_ == kDestroy);
						  m_parent->producer_sig_processed_ = true;
						  m_parent->produce_end_ = true;
				  ...
				  // now without lock
				  m_parent->produce_end_ = !m_next(&cell);
				  assert(cell != NULL || m_parent->produce_end_);
				  bool notify;
				  {
					  // lockscope
					  JTCSyncT<JTCMutex> lock(m_parent->mutex_);
					  if (!m_parent->produce_end_) {
						  m_parent->queue_.push(cell);
					  } else {
						  if (cell != NULL) m_parent->free_cells_.push(cell);
					  }
					  // put things into queue
					  notify = m_parent->nwait_consumer_ != 0;
				  }

				  if (notify) m_parent->consumer_cond_.broadcast();
			  }
		  }
	  };
#endif
	   ...
	   */
	  explicit HyperThreadedIter(size_t max_capacity = 8)
		  : producer_owned_(NULL),
			...
			out_data_(NULL) {}
	  /*! \brief destructor */
	  virtual ~HyperThreadedIter(void) {
		this->Destroy();
	  }
	  /*!
	  ...
	  virtual const DType &Value(void) const {
		assert(out_data_ != NULL);// << "Calling Value at beginning or end?";
		return *out_data_;
	  }
	  /*! \brief set the iterator before first location */
	  virtual void BeforeFirst(void) 
	  {
#ifdef USE_CX11_FEATURE
		...
		assert(!producer_sig_processed_);
...
#else
		auto is_processed = [this]()->bool{return producer_sig_processed_;};
		while(!is_processed())
		{
			consumer_cond_.wait(mutex_);
		}
#endif
...
#else
		if (notify) producer_cond_.signal();
#endif
	  }
...
#else
	  JTCCond producer_cond_;
	  JTCCond consumer_cond_;
#endif
	...
	template<typename DType>
	inline void HyperThreadedIter<DType>::Destroy(void) {
	...
	template<typename DType>
	inline void HyperThreadedIter<DType>::
	Init(Producer *producer, bool pass_ownership)
	{
	  assert(producer_owned_ == NULL);// << "can only call Init once";
		 ...
		 std::function<void()> beforefirst) 
	{
	  ...
	  producer_thread_ = std::thread(producer_fun);
	}
#else
	  producer_thread_ = new JTCProducerThread<DType>;
	  ((JTCProducerThread<DType>*)producer_thread_)->m_parent = this;
	  ((JTCProducerThread<DType>*)producer_thread_)->m_next = next;
	  ((JTCProducerThread<DType>*)producer_thread_)->m_beforefirst = beforefirst;
	  ((JTCProducerThread<DType>*)producer_thread_)->start();
#endif
	}

	template<typename DType>
	inline bool HyperThreadedIter<DType>::
	Next(DType **out_dptr) {
	  ...
	  auto consumer_begin = [this]()->bool {
		  return queue_.size() != 0 || produce_end_;
	  };
	  while(!consumer_begin())
	  {
		consumer_cond_.wait(mutex_);
	  }
	...
	template<typename DType>
	inline void HyperThreadedIter<DType>::Recycle(DType **inout_dptr) {
	  bool notify;
	  {
#ifdef USE_CX11_FEATURE
		std::lock_guard<std::mutex> lock(mutex_);		
