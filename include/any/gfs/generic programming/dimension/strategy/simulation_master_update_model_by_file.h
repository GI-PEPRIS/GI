...
#ifndef ANYWAY_RCF_DIMENSION_SIMULATION_MASTER_UPDATE_MODEL_BY_FILE_H
#define ANYWAY_RCF_DIMENSION_SIMULATION_MASTER_UPDATE_MODEL_BY_FILE_H
	...
	namespace rcf
	{
		namespace detail
		{
			namespace strategy
			{
				...
				// services
				//
				struct simulation_master_update_model_by_file : public default_strategy
				{
					//
					template
						<
					...
					static bool apply(Master &master, Model &model, GraphNode *running_node)
					{
						...
						try
						{
							...
							if(!running_node)//initialize
							{
								...
								string filepath = "";
								model_simulator->get_model_simulator_data_dir(&model, filepath);
								{
									char buffer[256];
								...
								masterimpl->UploadModelFile(0, filepath, fileUpload);
							}
							else//sync
							{
								typedef simulation_simulator<Model>::type Simulator;
								...
								masterimpl->UploadModelData(0, whichsimulationstep, pressure_LoopNumber, heat_LoopNumber);
								std::vector<string> filepaths;
								model_simulator->get_model_step_data_filepath(&model, filepaths);
								for(int i = 0; i < filepaths.size(); i++)
								{
									RCF::FileUpload fileUpload(filepaths[i]);
									masterimpl->UploadModelStepFile(0, filepaths[i], fileUpload); 
								}
								masterimpl->LoadModelCurrentStepData(0);
							}
							//
