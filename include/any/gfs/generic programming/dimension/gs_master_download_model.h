...
#ifndef ANYWAY_RCF_DIMENSION_GS_MASTER_DOWNLOAD_MODEL_H
#define ANYWAY_RCF_DIMENSION_GS_MASTER_DOWNLOAD_MODEL_H
...
#include "generic programming/dimension/strategy/simulation_master_download_model_by_file.h"
...
namespace anyway
{
	namespace rcf
	{
		...
		namespace detail
		{
			struct default_master_download_model
			{
		...
		namespace dispatch
		{
			template
				...
				>
			struct gs_master_download_model : detail::calculate_null
			{
				// not implemented
			...
			//
			template <typename Master, typename Model, typename GraphNode>
			struct gs_master_download_model<Master, Model, GraphNode, deltasoft_tag, simulation_model_simulator_basin_tag>
			{
				template<typename Strategy>
				static inline bool apply(Master &master, Model &model, GraphNode *running_node, Strategy &strategy)
				{
		...
		{
			struct gs_master_download_model
			{
				template<typename Master, typename Model, typename GraphNode, typename Strategy>
				static inline bool apply(Master &master, Model &model, GraphNode *running_node, Strategy &strategy)
				{
					return dispatch::gs_master_download_model<Master, Model, GraphNode>::apply(master, model, running_node, strategy);
				}
				...
				{
					return detail::default_master_download_model::apply(master, model, running_node);
				}
				...
				{
					return dispatch::gs_master_download_model<Master, Model, GraphNode>::apply(master, model, running_node, file_strategy);
				}
			};
		}
		...
		namespace resolve_variant
		{
			...
			struct gs_master_download_model
			{
				...
				static inline bool apply(Master &master, Model &model, GraphNode *running_node, Strategy &strategy)
				{
					return resolve_strategy::gs_master_download_model::apply(master, model, running_node, strategy);
				}
			};
		}
		...
		//
		template<typename Master, typename Model, typename GraphNode>
		bool gs_master_download_model(Master &master, Model &model, GraphNode *running_node)
		{
			return resolve_variant::gs_master_download_model
				<
				Master,
				Model,
				GraphNode
		...
		template<typename Master, typename Model, typename GraphNode, typename Strategy>
		bool gs_master_download_model(Master &master, Model &model, GraphNode *running_node, Strategy &strategy)
		{
			return resolve_variant::gs_master_download_model
				<
				Master,
