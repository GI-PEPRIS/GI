 ...
 */
#ifndef __ANYDB_ADO_ACCESSORIMPL_H__
#define __ANYDB_ADO_ACCESSORIMPL_H__
...
namespace AnyDB
{
	...
	class CAnyAdoRecordset;
	
	/*!
	*
	**/
	class ANYDB_CLASS CAnyAdoAccessorImpl : public CAnyAccessorImpl, public JTCMonitor
	{
		// Relatives
		friend class CAnyAccessorImpl;

		// For those records in memory respectively for each table
		typedef std::map<const std::string, CAnyAdoRecordset*> RecordsetMap;
	...
	protected:
		...
		*
		**/
		CAnyAdoAccessorImpl(const DBSourceDesc &source);

		/*!
		*
		**/
		virtual ~CAnyAdoAccessorImpl();
	...
	private:
		...
		// Current record set
		CAnyAdoRecordset *m_curRecordset;
