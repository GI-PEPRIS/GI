 ...
 */
#ifndef __ANYGDE_RCF_TASK_TRIANGLEFILTER_SERVANT_H__
#define __ANYGDE_RCF_TASK_TRIANGLEFILTER_SERVANT_H__
...
{
	// interface
	RCF_BEGIN(I_GdeTriangleFilterTask, "I_GdeTriangleFilterTask")
	...
	RCF_END(I_GdeTriangleFilterTask)

	/*!
	*
	**/
	class ANYGDE_CLASS CGdeTriangleFilterTaskServant : public CGdeTaskServantCommon
	{
		// callback
		typedef RcfClient<I_GdeTriangleFilterTask> CbClient;
		typedef boost::shared_ptr<CbClient> CbClientPtr;

		class MapTask : public CAnySignalTask<void()>, public JTCMonitor
		{
		public:
			MapTask(int taskcount, int index, CGdeTriangleFilterTaskServant *parent, MapKey *key, MapValue *value, \
				std::vector<long> &sequenceOrders, bool &ispremode, std::vector<int> &cmpcounts, CAnyCPUScheduler *scheduler);
			~MapTask();
			void DoExecuteTask();
			int m_taskcount;
			int m_index;
			CGdeTriangleFilterTaskServant *m_parent;
			MapKey *m_key;
			MapValue *m_value;
	...
	private:
		...
		*  trianglefilter
		**/
		void DoTriangleFilter(MapKey *key, MapValue *value);
