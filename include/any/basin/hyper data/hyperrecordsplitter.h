 ...
 */
#ifndef __ANYBASIN_HYPER_DATA_RECORD_SPLITTER_H__
#define __ANYBASIN_HYPER_DATA_RECORD_SPLITTER_H__

//
#include "hyperinputsplitbase.h"
...
namespace AnyBasin
{
	...
	class HyperRecordSplitter : public HyperInputSplitBase 
	{
	public:
		HyperRecordSplitter(HyperFileSystem *fs,
			const char *uri,
			unsigned rank,
			unsigned nsplit) {
				this->Init(fs, uri, 4);
	...
	protected:
		virtual size_t SeekRecordBegin(HyperStream *fi);
