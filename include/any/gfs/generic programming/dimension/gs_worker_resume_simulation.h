...
#ifndef ANYWAY_RCF_DIMENSION_GS_WORKER_RESUME_SIMULATION_H
#define ANYWAY_RCF_DIMENSION_GS_WORKER_RESUME_SIMULATION_H
...
namespace anyway
{
	namespace rcf
	{
		...
		namespace detail
		{
			struct default_worker_resume_simulation{
				template<typename Worker>
				static void apply(Worker &worker, long job_idx){
					...
					try
					{
						...
						workerimpl->ResumeSimulation(job_idx);
					}
					catch (...)
					{
		...
		namespace dispatch
		{
			template
				...
				>
			struct gs_worker_resume_simulation :  detail::calculate_null
			{
				// not implemented
			...
			//
			template <typename Worker>
			struct gs_worker_resume_simulation<Worker, deltasoft_tag> : public detail::default_worker_resume_simulation
			{
			};
		}
		...
		//
		template<typename Worker>
		void gs_worker_resume_simulation(Worker &worker, long job_idx)
		{
			dispatch::gs_worker_resume_simulation
				<
				Worker
				>::apply(worker, job_idx);
		}
