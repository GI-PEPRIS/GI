...
//
TF_API Status GetWindowedOutputSize(int64 input_size, int64 filter_size, int64 stride,
...
TF_API Status GetWindowedOutputSizeV2(int64 input_size, int64 filter_size,
...
TF_API Status GetWindowedOutputSizeVerbose(int64 input_size, int64 filter_size,
...
TF_API Status GetWindowedOutputSizeVerboseV2(int64 input_size, int64 filter_size,
                                      int64 dilation_rate, int64 stride,
                                      Padding padding_type, int64* output_size,
...
TF_API Status Get3dOutputSize(const std::array<int64, 3>& input,
...
// The V2 version computes the same outputs with arbitrary dilation_rate. For
// detailed equations, refer to the comments for GetWindowedOutputSizeV2().
TF_API Status Get3dOutputSizeV2(const std::array<int64, 3>& input,
                         const std::array<int64, 3>& window,
...
TF_API inline Status UnchangedShapeWithRankAtLeast(
...
// Transfers shape of input(0) to output(0), after asserting its rank <= <rank>.
TF_API inline Status UnchangedShapeWithRankAtMost(shape_inference::InferenceContext* c,
                                           int32 rank) {
  ShapeHandle out;
...
TF_API inline Status NoOutputs(shape_inference::InferenceContext* c) {
  return Status::OK();
}

// Shape function for ops that output a single scalar value.
TF_API inline Status ScalarShape(shape_inference::InferenceContext* c) {
  c->set_output(0, c->Scalar());
  return Status::OK();
}

// Shape function for binary ops where both inputs and the output match.
TF_API inline Status MergeBothInputsShapeFn(InferenceContext* c) {
  ShapeHandle out;
...
TF_API Status MakeShapeFromFormat(TensorFormat format, DimensionOrConstant N,
...
TF_API Status MatMulShape(shape_inference::InferenceContext* c);

// Shape function for Batched MatMul-like operations with broadcasting across
// batch dimensions.
TF_API Status BatchMatMulV2Shape(shape_inference::InferenceContext* c);

// Shape function for BiasAdd-like operations.
TF_API Status BiasAddShape(shape_inference::InferenceContext* c);

// Shape function for BiasAddGrad-like operations.
TF_API Status BiasAddGradShape(shape_inference::InferenceContext* c);

// Shape function for Conv2D-like operations that support explicit padding.
TF_API Status Conv2DShapeWithExplicitPadding(shape_inference::InferenceContext* c);

// Shape function for Conv2D-like operations that do not support explicit
// padding.
TF_API Status Conv2DShape(shape_inference::InferenceContext* c);

// Shape function for Conv3D-like operations.
TF_API Status Conv3DShape(shape_inference::InferenceContext* c);

// Shape function for DepthwiseConv2D-like operations.
TF_API Status DepthwiseConv2DNativeShape(shape_inference::InferenceContext* c);

// Shape function for AvgPool-like operations.
TF_API Status AvgPoolShape(shape_inference::InferenceContext* c);

// Shape function for FusedBatchNorm and FusedBatchNormV2 operations.
TF_API Status FusedBatchNormShape(shape_inference::InferenceContext* c);

// Shape function for FusedBatchNormV3 operations.
TF_API Status FusedBatchNormV3Shape(shape_inference::InferenceContext* c);

// Shape function for FusedBatchNormGrad and FusedBatchNormGradV2 operations.
TF_API Status FusedBatchNormGradShape(shape_inference::InferenceContext* c);

// Shape function for MaxPool-like operations.
TF_API Status MaxPoolShape(shape_inference::InferenceContext* c);

// Shape function for MaxPoolV2-like operations.
TF_API Status MaxPoolV2Shape(shape_inference::InferenceContext* c, int num_inputs);

// Shape function for 3D Pooling operations.
TF_API Status Pool3DShape(shape_inference::InferenceContext* c);

// Shape function for use with ops whose output shapes are unknown.
TF_API Status UnknownShape(shape_inference::InferenceContext* c);

// Shape function for reduction operations.
TF_API Status ReductionShape(shape_inference::InferenceContext* c);
...
TF_API Status ConcatShape(shape_inference::InferenceContext* c,
                   int num_inputs_to_concat);

// Shape function for concat operations.
TF_API Status ConcatV2Shape(shape_inference::InferenceContext* c);

TF_API Status QuantizedConcatV2Shape(InferenceContext* c, int num_inputs_to_concat);

// Shape function for binary operators that broadcast their inputs
// and with output to output_index.
// Note: out cannot be NULL.
TF_API Status BroadcastBinaryOpOutputShapeFnHelper(InferenceContext* c,
                                            ShapeHandle shape_x,
                                            ShapeHandle shape_y,
                                            ShapeHandle* out);
...
// Shape function for binary operators that broadcast their inputs
// and with output to output_index.
TF_API inline Status BroadcastBinaryOpOutputShapeFn(InferenceContext* c,
                                             int output_index) {
  ShapeHandle out;
  TF_RETURN_IF_ERROR(
...
TF_API inline Status BroadcastBinaryOpShapeFn(InferenceContext* c) {
  return BroadcastBinaryOpOutputShapeFn(c, 0);
}

// Shape function for random operations.
TF_API Status RandomShape(shape_inference::InferenceContext* c);

// Shape function for Slice opertaions.
TF_API Status SliceShape(shape_inference::InferenceContext* c);
