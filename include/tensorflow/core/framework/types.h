...
// clang-format off
#include "thrid_party/eigen3/unsupported/Eigen/CXX11/FixedPoint"
// clang-format on
...
TF_API_NO_C string DataTypeString(DataType dtype);
TF_API_NO_C string DeviceTypeString(const DeviceType& device_type);
TF_API_NO_C  string DataTypeSliceString(const DataTypeSlice dtypes);
...
class TF_CLASS DataTypeSet {
 private:
  const uint32 mask_;

  static const /*constexpr*/ uint32 kNumBits = 32;
...
// return false.
TF_API_NO_C bool DataTypeFromString(StringPiece sp, DataType* dt);
...
#define MATCH_TYPE_AND_ENUM(TYPE, ENUM)                 \
  ...
  struct DataTypeToEnum<TYPE> {                         \
    ...
    static const /*constexpr*/ DataType value = ENUM;             \
  };                                                    \
  template <>                                           \
