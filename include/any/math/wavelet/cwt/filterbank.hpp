...
namespace wavelet {
	...
	*
	**/
	 struct ProcessThreadParameter
	{
		int m_from;
		int m_to;
		std::vector<double> &m_values;
		wavelet::Attribute<bool> &m_rescale;
		arma::cx_vec &m_sig_spectral;
		arma::cx_mat &m_scalogram;
		std::vector< std::shared_ptr<wavelet::Wavelet> > &m_wavelets;

		ProcessThreadParameter(int from, int to, std::vector<double> &values, wavelet::Attribute<bool> &rescale, \
			arma::cx_vec &sig_spectral, arma::cx_mat &scalogram,std::vector< std::shared_ptr<wavelet::Wavelet> > &wavelets) : \
			m_from(from), m_to(to), m_values(values), m_rescale(rescale), m_sig_spectral(sig_spectral), m_scalogram(scalogram), m_wavelets(wavelets)
		{
		}
	};
    /**
...
#pragma mark > Offline Estimation
		...
		*
		**/
        arma::cx_mat process_thread(std::vector<double> values);
		void do_process_thread(const ProcessThreadParameter &parameter);
