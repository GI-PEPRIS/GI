...
#ifndef ANYWAY_RCF_DIMENSION_GS_MASTER_SET_TORTOISE_IDX_H
#define ANYWAY_RCF_DIMENSION_GS_MASTER_SET_TORTOISE_IDX_H
...
namespace anyway
{
	namespace rcf
	{
		...
		namespace detail
		{
			struct default_master_set_tortoise_idx
			{
				...
				static void apply(Master &master, long job_idx)
				{
					try
					{
						...
						masterimpl->SetTortoiseIdx(job_idx);
					}
					catch (...)
					{
					}
		...
		namespace dispatch
		{
			template
				...
				>
			struct gs_master_set_tortoise_idx :  detail::calculate_null
			{
				// not implemented
			...
			//
			template <typename Master>
			struct gs_master_set_tortoise_idx<Master, deltasoft_tag> : public detail::default_master_set_tortoise_idx
			{
			};
		}
		...
		//
		template<typename Master>
		void gs_master_set_tortoise_idx(Master &master, long job_idx)
		{
			return dispatch::gs_master_set_tortoise_idx
				<
				Master
