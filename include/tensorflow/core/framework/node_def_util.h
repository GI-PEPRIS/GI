...
TF_API_NO_C string SummarizeNode(const Node& node);
TF_API_NO_C  string SummarizeNodeDef(const NodeDef& node_def);
TF_API_NO_C string SummarizeAttrs(const NodeDef& node_def);
TF_API_NO_C string SummarizeAttrsHelper(AttrSlice attrs, StringPiece device);

// Produces a formatted string pattern from the node which can uniquely identify
// this node upstream to produce an informative error message. The pattern
// followed is: {{node <node_name>}}
TF_API_NO_C string FormatNodeForError(const Node& node);
TF_API_NO_C string FormatNodeDefForError(const NodeDef& node_def);
TF_API_NO_C string FormatNodeDefForError(
...
TF_API_NO_C void MergeDebugInfo(const NodeDebugInfo& from, Node* to);
TF_API_NO_C void MergeDebugInfo(const NodeDebugInfo& from, NodeDef* to);
TF_API_NO_C void MergeDebugInfo(const NodeDef& from, NodeDef* to);
...
TF_API_NO_C void AddNodeAttr(StringPiece name, const AttrValue& value, NodeDef* node_def);
TF_API_NO_C void AddNodeAttr(StringPiece name, StringPiece value, NodeDef* node_def);
TF_API_NO_C void AddNodeAttr(StringPiece name, const char* value, NodeDef* node_def);
TF_API_NO_C void AddNodeAttr(StringPiece name, int32 value, NodeDef* node_def);
TF_API_NO_C void AddNodeAttr(StringPiece name, int64 value, NodeDef* node_def);
TF_API_NO_C void AddNodeAttr(StringPiece name, float value, NodeDef* node_def);
TF_API_NO_C void AddNodeAttr(StringPiece name, double value, NodeDef* node_def);
TF_API_NO_C void AddNodeAttr(StringPiece name, bool value, NodeDef* node_def);
TF_API_NO_C void AddNodeAttr(StringPiece name, DataType value, NodeDef* node_def);
TF_API_NO_C void AddNodeAttr(StringPiece name, const PartialTensorShape& value,
                 NodeDef* node_def);
TF_API_NO_C void AddNodeAttr(StringPiece name, const Tensor& value, NodeDef* node_def);
TF_API_NO_C void AddNodeAttr(StringPiece name, const TensorProto& value, NodeDef* node_def);
TF_API_NO_C void AddNodeAttr(StringPiece name, const NameAttrList& value,
                 NodeDef* node_def);
TF_API_NO_C void AddNodeAttr(StringPiece name, gtl::ArraySlice<StringPiece> value,
                 NodeDef* node_def);
TF_API_NO_C void AddNodeAttr(StringPiece name, gtl::ArraySlice<const char*> value,
                 NodeDef* node_def);
TF_API_NO_C void AddNodeAttr(StringPiece name, gtl::ArraySlice<string> value,
                 NodeDef* node_def);
TF_API_NO_C void AddNodeAttr(StringPiece name, gtl::ArraySlice<int32> value,
                 NodeDef* node_def);
TF_API_NO_C void AddNodeAttr(StringPiece name, gtl::ArraySlice<int64> value,
                 NodeDef* node_def);
TF_API_NO_C void AddNodeAttr(StringPiece name, gtl::ArraySlice<float> value,
                 NodeDef* node_def);
TF_API_NO_C void AddNodeAttr(StringPiece name, gtl::ArraySlice<bool> value,
                 NodeDef* node_def);
TF_API_NO_C void AddNodeAttr(StringPiece name, const std::vector<bool>& value,
                 NodeDef* node_def);
TF_API_NO_C void AddNodeAttr(StringPiece name, gtl::ArraySlice<DataType> value,
                 NodeDef* node_def);
TF_API_NO_C void AddNodeAttr(StringPiece name, gtl::ArraySlice<TensorShape> value,
                 NodeDef* node_def);
TF_API_NO_C void AddNodeAttr(StringPiece name, gtl::ArraySlice<PartialTensorShape> value,
                 NodeDef* node_def);
TF_API_NO_C void AddNodeAttr(StringPiece name, gtl::ArraySlice<TensorShapeProto> value,
                 NodeDef* node_def);
TF_API_NO_C void AddNodeAttr(StringPiece name, gtl::ArraySlice<Tensor> value,
                 NodeDef* node_def);
TF_API_NO_C void AddNodeAttr(StringPiece name, gtl::ArraySlice<NameAttrList> value,
                 NodeDef* node_def);
...
TF_API_NO_C void AddAttr(StringPiece name, const AttrValue& value, AttrValueMap* map);
TF_API_NO_C void AddAttr(StringPiece name, bool value, AttrValueMap* map);

class TF_CLASS AttrSlice {
 public:
...
TF_API_NO_C bool HasNodeAttr(const NodeDef& node_def, StringPiece attr_name);

// Look up the attr with name attr_name and set *value to its value.  If no
// attr with attr_name is found in node_def, or the attr does not have
// a matching type, a non-ok status will be returned.
TF_API_NO_C Status GetNodeAttr(const AttrSlice& attrs, StringPiece attr_name,
...
TF_API_NO_C Status GetNodeAttr(
...
TF_API_NO_C bool GetNodeAttrSimple(const AttrSlice& attrs, StringPiece attr_name,
                       string* value);  // type: "string"
TF_API_NO_C bool GetNodeAttrSimple(const AttrSlice& attrs, StringPiece attr_name,
...
TF_API_NO_C const string& GetNodeAttrString(const AttrSlice& attrs, StringPiece attr_name);

// Computes the input type for a specific node input.
// REQUIRES: ValidateOpDef(op_def).ok()
TF_API_NO_C Status InputTypeForNode(const NodeDef& node_def, const OpDef& op_def,
                        int input_port, DataType* input_type);
// Computes the input types for a specific node.
// REQUIRES: ValidateOpDef(op_def).ok()
TF_API_NO_C Status InputTypesForNode(const NodeDef& node_def, const OpDef& op_def,
                         DataTypeVector* inputs);
// Computes the output type for a specific node output.
// REQUIRES: ValidateOpDef(op_def).ok()
TF_API_NO_C Status OutputTypeForNode(const NodeDef& node_def, const OpDef& op_def,
                         int output_port, DataType* output_type);
// Computes the output types for a specific node.
// REQUIRES: ValidateOpDef(op_def).ok()
TF_API_NO_C Status OutputTypesForNode(const NodeDef& node_def, const OpDef& op_def,
                          DataTypeVector* outputs);
TF_API_NO_C Status OutputTypesForNode(const AttrSlice& attrs, const OpDef& op_def,
                          DataTypeVector* outputs);

// Computes the input and output types for a specific node.
// REQUIRES: ValidateOpDef(op_def).ok()
TF_API_NO_C Status InOutTypesForNode(const NodeDef& node_def, const OpDef& op_def,
                         DataTypeVector* inputs, DataTypeVector* outputs);
// Computes the number of outputs for a specific node.
// REQUIRES: ValidateOpDef(op_def).ok()
TF_API_NO_C Status NumOutputsForNode(const NodeDef& node_def, const OpDef& op_def,
                         int* num_outputs);
...
// etc.
TF_API_NO_C Status ValidateNodeDef(const NodeDef& node_def, const OpDef& op_def);
...
typedef gtl::FlatMap<StringPiece, std::pair<int, int>, hash<StringPiece>>
    NameRangeMap;
TF_API_NO_C Status NameRangesForNode(const AttrSlice& attrs, const OpDef& op_def,
                         NameRangeMap* inputs, NameRangeMap* outputs);
TF_API_NO_C Status NameRangesForNode(const Node& node, const OpDef& op_def,
                         NameRangeMap* inputs, NameRangeMap* outputs);

// Adds default values to *node_def for unspecified attrs from op_def.
TF_API_NO_C void AddDefaultsToNodeDef(const OpDef& op_def, NodeDef* node_def);
...
TF_API_NO_C Status ValidateExternalNodeDefSyntax(const NodeDef& node_def);
...
TF_API_NO_C Status AttachDef(const Status& status, const NodeDef& node_def,
                 bool allow_multiple_formatted_node = false);
TF_API_NO_C Status AttachDef(const Status& status, const Node& node,
                 bool allow_multiple_formatted_node = false);
...
TF_API_NO_C Status AddPrefixAndSuffixToNode(StringPiece prefix, StringPiece suffix,
                                NodeDef* node_def);
}  // namespace tensorflow
