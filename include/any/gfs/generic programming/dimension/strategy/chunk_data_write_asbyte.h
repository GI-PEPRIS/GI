...
#ifndef ANYWAY_RCF_DIMENSION_CHUNK_DATA_WRITE_ASBYTE_H
#define ANYWAY_RCF_DIMENSION_CHUNK_DATA_WRITE_ASBYTE_H
	...
	namespace rcf
	{
		namespace detail
		{
			namespace strategy
			{
				...
				// services
				//
				struct chunk_data_write_asbyte : public default_strategy
				{
					//
					//
					typedef strategy_tag_chunk_data_write_asbyte type;
					...
					static inline bool apply(MasterServant const& masterservant, ChunkServant const& chunkservant, ChunkFile const& chunkfile, ChunkData const& chunkdata, ChunkSize const& chunksize)
					{
						...
						try
						{
							...
							// assure whether normally passed
							{
								...
								{
									int bytesSentOnce = 512 * 1024;
									for(long long j = 0; j < chunksize[i]; j = j + bytesSentOnce)
									{
										int readlength = j + bytesSentOnce > chunksize[i] ? chunksize[i] - j : bytesSentOnce;
										std::vector<char> data;
										data.resize(readlength);
										char* datachar = data.data();
										char* chunkdatachar = (char*)(chunkdata[i]) + j;
										memcpy((void*)datachar, chunkdatachar, readlength);
										chunkimpl->WriteData(uploadfilepath, data, readlength, offset, offset == 0 ? CAnyFileBasic::ANY_FILE_WRITE : CAnyFileBasic::ANY_FILE_ALL);
										offset = offset + (long long)readlength;
									}
								}
							}
						}
