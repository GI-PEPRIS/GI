...
typedef std::unordered_map<string, Creator> OpGradFactory;//herman
static OpGradFactory* gradient_factory = 0;

TF_API OpGradFactory* GetOpGradFactory();
TF_API bool RegisterOp(const string& op, Creator func);

// Returns OK the gradient creator for the "op" is found (may be
// nullptr if REGISTER_OP_NO_GRADIENT is used.
TF_API Status GetOpGradientCreator(const string& op, Creator* creator);
};  // namespace gradient
